	...
	for y := 0; input.Scan(); y++ {
		...
		for x, u := range input.Text() {
			...
			case 71: // GOBLIN FOUND
				//if x == 16 && y == 21 {
				//	players[coordinate{x,y}] = &player{ class: goblin, attackPower: 3, hitPoints: 3 }
				//} else {
...
func locateTarget(startLocation coordinate) (route []coordinate) {
	...
	for len(openList) > 0 {
		...
		if (currentNode.steps > shortestRoute && shortestRoute != 0) || board[currentNode.location] == wall {
			continue
		}
		...
		for _, neighbour := range neighbours {
			...
			if _, ok := closedList[ coordinate { currentNode.location.x + neighbour.x, currentNode.location.y + neighbour.y }]; ok {
				continue
			}
