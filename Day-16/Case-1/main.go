...
func main() {
	...
	for s := 0; s < len(samples); s++ {
		...
		if checkIfEqual(samples[s].registerAfter, addr(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, addi(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, mulr(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, muli(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, banr(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, bani(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, borr(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, bori(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, setr(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, seti(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, gtir(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, gtri(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, gtrr(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, eqir(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, eqri(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
		if checkIfEqual(samples[s].registerAfter, eqrr(samples[s].registerBefore, samples[s].instruction)) {
			sampleCounter++
		}
