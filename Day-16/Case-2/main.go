	...
	for len(numberCodes) > 0 {
		...
		for n := 0; n < len(numberCodes); n++ {
			...
			for s := 0; s < len(samples); s++ {
				...
				if samples[s].instruction[0] != numberCodes[n] {
					continue
				}
				...
				for k, v := range knownOpcodes {
					if v.found == false {
						...
						if k == "addr" && checkIfEqual(samples[s].registerAfter, addr(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "addi" && checkIfEqual(samples[s].registerAfter, addi(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "mulr" && checkIfEqual(samples[s].registerAfter, mulr(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "muli" && checkIfEqual(samples[s].registerAfter, muli(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "banr" && checkIfEqual(samples[s].registerAfter, banr(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "bani" && checkIfEqual(samples[s].registerAfter, bani(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "borr" && checkIfEqual(samples[s].registerAfter, borr(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "bori" && checkIfEqual(samples[s].registerAfter, bori(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "setr" && checkIfEqual(samples[s].registerAfter, setr(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "seti" && checkIfEqual(samples[s].registerAfter, seti(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "gtir" && checkIfEqual(samples[s].registerAfter, gtir(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "gtri" && checkIfEqual(samples[s].registerAfter, gtri(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "gtrr" && checkIfEqual(samples[s].registerAfter, gtrr(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "eqir" && checkIfEqual(samples[s].registerAfter, eqir(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "eqri" && checkIfEqual(samples[s].registerAfter, eqri(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
						if k == "eqrr" && checkIfEqual(samples[s].registerAfter, eqrr(samples[s].registerBefore, samples[s].instruction)) {
							codesCounter[k]++
						}
					}
				}
