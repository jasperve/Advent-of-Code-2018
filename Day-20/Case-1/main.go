...
func main() {
	...
	for _, distance := range distances {
		if distance.stepsTaken/2 > maxNumDoors { maxNumDoors = distance.stepsTaken/2 }
		if distance.stepsTaken/2 >= 1000 && grid[distance.y][distance.x] == room { numRooms++ }
	}
	...
	for c := 0; c < len(input); c++ {
		...
		case 40: // (
			...
			for b := c+1; b < len(input); b++ {
				if input[b] == 40 { numBracketsOpened++	}
				if input[b] == 41 { numBracketsOpened-- }
			...
			for x := -1; x <= 1; x++ {
				if (y == -1 && x == 0) || (y == 0 && (x == -1 || x == 1)) || (y == 1 && x == 0) {
					...
					for c := 0; c < len(closedList); c++ {
						if closedList[c].x == currentCoordinate.x + x && closedList[c].y == currentCoordinate.y + y { continue XLOOP }
					}

					if grid[currentCoordinate.y+y][currentCoordinate.x+x] == wall { continue }
