...
func main() {
	...
	for _, section := range sections {
		...
		for _, subsection := range subsections {
			...
			groupsOriginal = append(groupsOriginal, &newGroup)
...
OUTER:
	for {
		...
		for g := 0; g < len(groupsOriginal); g++ {
			...
			for k := range groupsOriginal[g].weaknesses {
				newGroup.weaknesses[k] = true
			}
			if newGroup.system == "Immune System" {
		...
		for {
			...
			for g := 0; g < len(groups); g++ {
				amountUnitsBefore += groups[g].units
			}
			...
			if !immuneSystemFound {
				fmt.Printf("Boosting immunity system with %v results in a infection winning with %v units left!\n", boost, amountUnitsLeft)
				boost++
				break
			} else if !infectionFound {
				fmt.Printf("Boosting immunity system with %v results in a immune system winning with %v units left!\n", boost, amountUnitsLeft)
				boost++
				break OUTER
			}
